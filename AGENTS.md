...
## 数据库表
- `enterprises` - 企业
- `enterprise_members` - 企业成员（user_id + role + enterprise_id）
- `categories` - 话术分类（含 enterprise_id）
- `tags` - 标签（含 enterprise_id）
- `knowledge_entries` - 知识库条目（含 enterprise_id）
- `knowledge_entry_tags` - 条目-标签多对多
- `entry_versions` - 条目版本历史
- `entry_comments` - 条目评论（支持合并到答案）
- `qa_history` - 问答历史记录（含 enterprise_id）
...
18. `PUT /api/knowledge/[id]/use` - 记录使用次数（30秒防重复）
19. `GET/POST /api/enterprises` - 企业列表/创建
20. `POST /api/enterprises/join` - 通过邀请码加入企业
21. `GET /api/supabase-config` - 获取 Supabase Auth 配置

## 认证与企业隔离
- 登录方式：邮箱（Supabase Auth，autoconfirm）
- 请求头：`x-session`（access_token）、`x-enterprise-id`（当前企业ID）
- 所有业务 API 均需登录，数据按 enterprise_id 隔离
- 用户可属于多个企业，前端 localStorage 存储当前企业 ID
